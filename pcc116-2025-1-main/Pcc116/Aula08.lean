...
-- total 
---- casamento de padrão deve cobrir todas as possibilidades
---- recursão deve ser feita somente sobre argumentos menores
...
lemma plus_add (n m : ℕ) : n + m = n .+. m := by 
  induction n with 
  | zero =>
    simp [plus]
  | succ n' IH =>
...
#eval zipp [1,2,3] [4,5,6,7,8] 

def Vec.zip {A B : Type}{n} 
  (xs : Vec A n)
  (ys : Vec B n) : Vec (A × B) n := 
  match xs, ys with 
...
#eval String.toNat? "12"
#eval String.toNat? "aa"
...
#eval Finite.beq fakeBool fakeTrue fakeTrue 
