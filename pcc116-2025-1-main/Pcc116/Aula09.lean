...
-- ** fuel ≠ 0, calcule o resultado fazendo 
--              chamada recursiva sobre fuel - 1
/-
...
-- ∀ n, P n ≃ ∀ n, (∀ k, k < n → P k) → P n 

def strong_induction (P : ℕ → Type) 
  : (Step : ∀ m, (∀ k, k < m → P k) → P m) → 
    ∀ n, P n  := by 
  intros IH n  
...
def divF (n : ℕ)
         (divRec : (n' : ℕ) → n' < n → ℕ → ℕ) (m : ℕ) : ℕ :=
  if h : 0 < m ∧ m ≤ n then
    divRec (n - m) (div_rec h) m + 1
  else
    0
...
| DomEq : ∀ n, n ≠ 0 → Dom n n 
| DomGt : ∀ n m, n > m → Dom (n - m) m → Dom n m
...
def dom (n m : ℕ) (H : m ≠ 0) : Dom n m := 
  match Nat.decide n m with 
  | NatDec.LT _ _ H1 => Dom.DomLt n m H1
  | NatDec.EQ _ => Dom.DomEq n H  
  | NatDec.GT _ _ H1 =>
    Dom.DomGt n m H1 (dom (n - m) m H)
